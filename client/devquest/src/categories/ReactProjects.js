...
export default function ReactProjects() {
    ...
    useEffect(() => {fetch("http://localhost:4000/projects")
        .then((response) => {
            if (!response.ok) {
    ...
    return (
        ...
        {
            projects.filter(p => p.techId === 4).map(p =>
            <div className='card-frame'>
                <div className='project-card'>
                    <div className='card-image' style={{ backgroundImage: `${p.imgSrc}` }}></div>
                    <h1 className='card-h1'>{p.name}</h1>
                    <div className='my-p'>
                        <p><b>Category : {p.name}</b></p>
